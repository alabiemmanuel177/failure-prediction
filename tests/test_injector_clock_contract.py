...
def test_signal_fault_schedule_arms_from_retained_goal_event():
    source = (
        ROOT / "ros_ws/src/failure_experiment/failure_experiment/signal_proxy.py"
    ).read_text(encoding="utf-8")
    body = source.split("def on_event", 1)[1].split("def on_command", 1)[0]
    assert 'values.get("event_type") != "goal_dispatched"' in body
    assert 'values.get("run_id") != str(self.values["run_id"])' in body
    assert "schedule.arm(stamp_seconds(stamp))" in body
    assert 'publish_event("injection_planned", stamp)' in body
